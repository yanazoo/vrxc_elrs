...
### RotorHazard プラグインのインストール（このフォーク版）

タイマーに RotorHazard v4.1.0 以降がインストールされていることを確認してください。

#### 通常の Raspberry Pi（標準パス）

```bash
cd /home/pi/RotorHazard/src/server/custom_plugins
git clone https://github.com/yanazoo/vrxc_elrs.git temp_clone
cp -r temp_clone/custom_plugins/vrxc_elrs ./
rm -rf temp_clone
sudo systemctl restart rotorhazard
```

#### NuclearHazard ハードウェア

NuclearHazard では RotorHazard のプラグインフォルダーが `/home/NuclearHazard/rh-data/plugins/` です。
...
**プラグインの更新（既存インストールを最新版に更新）：**

```bash
wget -qO /home/NuclearHazard/rh-data/plugins/vrxc_elrs/__init__.py https://raw.githubusercontent.com/yanazoo/vrxc_elrs/master/custom_plugins/vrxc_elrs/__init__.py
wget -qO /home/NuclearHazard/rh-data/plugins/vrxc_elrs/elrs_backpack.py https://raw.githubusercontent.com/yanazoo/vrxc_elrs/master/custom_plugins/vrxc_elrs/elrs_backpack.py
sudo systemctl restart rotorhazard
```

> [!NOTE]
> インストール後、RotorHazard を再起動すると設定画面に「ELRS バックパック 一般設定」と「ELRS バックパック OSD 設定」パネルが表示されます。
...
### Heat Name Column : INT

ヒート名を表示する列位置。

### Class Name Column : INT

クラス名を表示する列位置。

### Event Name Column : INT

イベント名を表示する列位置。

### Announcement Column : INT

アナウンスを表示する列位置。

### Race Status Column : INT

レースステータスメッセージ（ステージング・スタート・終了等）を表示する列位置。

### Current Lap/Position Column : INT

現在のラップと順位を表示する列位置。

### Lap/Gap Results Column : INT

ラップまたはギャップタイムを表示する列位置。

### Results Column : INT

パイロットのレース後の統計を表示する列位置。
